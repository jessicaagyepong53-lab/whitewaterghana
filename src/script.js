...
const SALES_ADD_AUDIT_KEY = 'ww_sales_add_audit_log_v1';
...
function getInvoiceTxnKey(inv) {
	if (!inv || typeof inv !== 'object') return '';
	return String(inv.clientTxnId || inv.client_txn_id || inv.idempotencyKey || inv.idempotency_key || '').trim();
}
...
function getOrderTxnKey(ord) {
	if (!ord || typeof ord !== 'object') return '';
	return String(ord.clientTxnId || ord.client_txn_id || ord.idempotencyKey || ord.idempotency_key || '').trim();
}

function dedupeInvoiceRows(rows) {
	const list = Array.isArray(rows) ? rows.filter((inv) => inv && inv.id) : [];
	const byId = new Map();
	...
	for (const inv of byId.values()) {
		const txn = getInvoiceTxnKey(inv);
		if (!txn) {
			noTxn.push(inv);
			continue;
		}
		byTxn.set(txn, pickNewerRecord(byTxn.get(txn), inv));
	}
	return [...byTxn.values(), ...noTxn];
}

function dedupeOrderRows(rows) {
	const list = Array.isArray(rows) ? rows.filter((ord) => ord && ord.id) : [];
	const byId = new Map();
	for (const ord of list) {
		const id = String(ord.id || '').trim();
		if (!id) continue;
		byId.set(id, pickNewerRecord(byId.get(id), ord));
	}
	const byTxn = new Map();
	const noTxn = [];
	for (const ord of byId.values()) {
		const txn = getOrderTxnKey(ord);
		if (!txn) {
			noTxn.push(ord);
			continue;
		}
		byTxn.set(txn, pickNewerRecord(byTxn.get(txn), ord));
	}
	return [...byTxn.values(), ...noTxn];
}

function getSalesAddAuditEntries() {
	try {
		const raw = localStorage.getItem(SALES_ADD_AUDIT_KEY);
...
function resolveSalesAuditActor() {
	let user = null;
	try {
		if (window.__wwCurrentUser) user = window.__wwCurrentUser;
		else if (typeof getCachedSessionUser === 'function') user = getCachedSessionUser();
	} catch (_e) {
		user = null;
	}
	const name = String(user?.name || localStorage.getItem('ww_user_name') || '').trim() || 'Unknown';
	const email = String(user?.email || localStorage.getItem('ww_user_email') || '').trim().toLowerCase();
	const role = String(user?.role || localStorage.getItem('ww_user_role') || '').trim().toLowerCase();
	const canEditDelete = typeof user?.canEditDelete === 'boolean' ? user.canEditDelete : readStoredCanEditDelete();
	return { name, email, role, canEditDelete };
}

function getPrivilegedActionAuditEntries() {
	try {
...
function summarizeAuditRecord(recordData) {
	...
	const id = String(recordData.id || recordData.invoice_code || recordData.order_code || recordData.batch_code || '').trim();
	const primary = String(recordData.customer || recordData.name || recordData.product || recordData.description || '').trim();
	if (id && primary) return `${id} - ${primary}`;
	if (id) return id;
	if (primary) return primary;
	return '';
}

function mergePrivilegedActionAuditEntries(localEntries, serverEntries) {
	const map = new Map();
	const upsert = (row) => {
...
async function mergeSyncPrivilegedActionAuditLog(localEntries) {
	let serverEntries = [];
	try {
		const res = await fetch(API_BASE + '/api/app-data/' + encodeURIComponent(PRIVILEGED_ACTION_AUDIT_KEY), {
			credentials: 'include',
			cache: 'no-store',
		});
		if (res.ok) {
			const json = await res.json();
			if (Array.isArray(json?.data)) serverEntries = json.data;
		}
	} catch (_e) { /* ignore */ }
	const merged = mergePrivilegedActionAuditEntries(localEntries, serverEntries);
	try {
		localStorage.setItem(PRIVILEGED_ACTION_AUDIT_KEY, JSON.stringify(merged));
	} catch (_e) { /* ignore */ }
	await syncToServer(PRIVILEGED_ACTION_AUDIT_KEY, merged);
}

function appendPrivilegedActionAudit(actionInput) {
	const action = actionInput && typeof actionInput === 'object' ? actionInput : {};
	const actor = resolveSalesAuditActor();
	if (!shouldTrackPrivilegedAudit(actor)) return false;
	const marker = {
		id: `pa-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`,
		timestamp: new Date().toISOString(),
		action: String(action.action || '').trim() || 'update',
		module: String(action.module || '').trim() || 'general',
		entity: String(action.entity || '').trim(),
		recordId: String(action.recordId || '').trim(),
		summary: String(action.summary || '').trim(),
		details: String(action.details || '').trim(),
		month: String(action.month || '').trim() || String(currentSalesMonth || '').trim(),
	...
	mergeSyncPrivilegedActionAuditLog(next).catch(() => { /* keep local log on sync failure */ });
	return true;
}

function appendSalesAddAuditMarker(entryInput) {
	const entry = entryInput && typeof entryInput === 'object' ? entryInput : {};
	const txnId = String(entry.txnId || '').trim();
	if (!txnId) return false;
	const actor = resolveSalesAuditActor();
	const stamp = new Date().toISOString();
	const marker = {
		id: `sa-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`,
		type: String(entry.type || 'invoice_add').trim() || 'invoice_add',
		month: String(entry.month || '').trim() || String(currentSalesMonth || '').trim(),
		txnId,
		timestamp: stamp,
		userName: actor.name,
		userEmail: actor.email,
	...
	const existing = getSalesAddAuditEntries();
	const already = existing.some((row) => String(row?.txnId || '') === marker.txnId && String(row?.type || '') === marker.type);
	if (already) return false;
	const next = [marker, ...existing].slice(0, SALES_ADD_AUDIT_LIMIT);
	try {
		localStorage.setItem(SALES_ADD_AUDIT_KEY, JSON.stringify(next));
	} catch (_e) { /* ignore */ }
	mergeSyncSalesAddAuditLog(next).catch(() => { /* keep local log on sync failure */ });
	return true;
}

function mergeSalesAddAuditEntries(localEntries, serverEntries) {
	const map = new Map();
	const toTs = (value) => {
	...
	const upsert = (row) => {
		if (!row || typeof row !== 'object') return;
		const txn = String(row.txnId || '').trim();
		...
		map.set(key, toTs(row.timestamp) >= toTs(existing.timestamp) ? row : existing);
	};
	(localEntries || []).forEach(upsert);
	(serverEntries || []).forEach(upsert);
	return Array.from(map.values())
		.sort((a, b) => toTs(b?.timestamp) - toTs(a?.timestamp))
		.slice(0, SALES_ADD_AUDIT_LIMIT);
}

async function mergeSyncSalesAddAuditLog(localEntries) {
	let serverEntries = [];
	try {
		const res = await fetch(API_BASE + '/api/app-data/' + encodeURIComponent(SALES_ADD_AUDIT_KEY), {
			credentials: 'include',
			cache: 'no-store',
		});
		if (res.ok) {
	...
	} catch (_e) { /* ignore; fallback to local */ }
	const merged = mergeSalesAddAuditEntries(localEntries, serverEntries);
	try {
		localStorage.setItem(SALES_ADD_AUDIT_KEY, JSON.stringify(merged));
	} catch (_e) { /* ignore */ }
	await syncToServer(SALES_ADD_AUDIT_KEY, merged);
}
...
function mergeSalesMonthPayloads(localPayload, incomingPayload, month) {
	...
	const mergedInvoicesRaw = dedupeInvoiceRows([...localInv, ...incomingInv]);
	const mergedOrdersRaw = dedupeOrderRows([...localOrd, ...incomingOrd]);
	...
	const invoices = mergedInvoicesRaw.filter((inv) => {
	...
	const salesOrders = mergedOrdersRaw.filter((ord) => {
...
function setSalesSyncWarning(message) {
	...
	try {
		if (!text) localStorage.removeItem(SALES_SYNC_WARNING_KEY);
		else localStorage.setItem(SALES_SYNC_WARNING_KEY, text);
	} catch (_e) { /* ignore */ }
	try {
		document.dispatchEvent(new CustomEvent('ww-sales-sync-warning', { detail: { message: text } }));
	} catch (_e) { /* ignore */ }
}

function getSalesSyncWarning() {
	try { return String(localStorage.getItem(SALES_SYNC_WARNING_KEY) || '').trim(); } catch (_e) { return ''; }
}
...
function putAppDataKeyToServer(key, data, logResult = true) {
	...
	.then(res => {
		if (!res.ok && /^ww_sales_\d{4}-\d{2}$/.test(String(key || '')) && res.status === 409) {
			res.clone().json().then((payload) => {
				const serverMessage = String(payload?.message || '').trim();
				setSalesSyncWarning(serverMessage || 'Sales sync rejected due to duplicate transaction key conflict.');
			}).catch(() => {
				setSalesSyncWarning('Sales sync rejected due to duplicate transaction key conflict.');
			});
		}
		if (logResult) {
		...
		if (res.ok && /^ww_sales_\d{4}-\d{2}$/.test(String(key || ''))) setSalesSyncWarning('');
		return res.ok;
	})
	.catch(err => {
...
function moveAppDataDeleteToTrash(module, recordData, restoreMeta) {
	...
	appendPrivilegedActionAudit({
		...
		entity: String(restoreMeta?.arrayPath || restoreMeta?.kind || '').trim(),
		recordId: String(recordData?.id || recordData?.invoice_code || recordData?.order_code || '').trim(),
		summary: summarizeAuditRecord(recordData),
		details: `Deleted from ${String(restoreMeta?.key || '').trim() || String(module || '').trim()}`,
	});
...
function mergeServerSalesIntoMemory(serverData, targetMonth) {
	...
	const localInvTxnKeys = new Set(salesModuleData.invoices.map((inv) => getInvoiceTxnKey(inv)).filter(Boolean));
	const localOrdTxnKeys = new Set(salesModuleData.salesOrders.map((ord) => getOrderTxnKey(ord)).filter(Boolean));
	...
	serverOrders.forEach((ord) => {
		...
		const txn = getOrderTxnKey(ord);
		if (!localOrdIds.has(ordId) && !(txn && localOrdTxnKeys.has(txn))) {
			...
			if (txn) localOrdTxnKeys.add(txn);
			changed = true;
		}
	});
	if (changed) {
...
function parseAccountingProductionQty(descriptionText) {
	const text = String(descriptionText || '');
	if (!text) return 0;
	const regex = /(\d+(?:\.\d+)?)\s*(packaging\s*bags?|bags?|rolls?|tnt)\b/ig;
	let match;
	let total = 0;
...
function parseDateToStartOfDayMs(rawDate) {
	const parsed = parseDateLikeToMs(rawDate);
...
function collectInventoryMonthsForReports() {
	const months = new Set();
	try {
		const fromIndex = JSON.parse(localStorage.getItem('ww_inventory_months') || '[]');
		if (Array.isArray(fromIndex)) {
			for (const month of fromIndex) {
				if (/^\d{4}-\d{2}$/.test(String(month || ''))) months.add(String(month));
			}
		}
	...
	for (let i = 0; i < localStorage.length; i += 1) {
		...
		const m = /^ww_inventory_(\d{4}-\d{2})$/.exec(key);
		if (m) months.add(m[1]);
	}
...
function buildProductionRowsFromInventoryAndAccounting(options = {}) {
	const filterStart = options.filterStart || null;
	const filterEnd = options.filterEnd || null;
	const accountingData = options.accountingData || { ledger: [] };

	const inRangeMs = (ms) => {
		if (!Number.isFinite(ms)) return false;
		if (filterStart && ms < new Date(filterStart).setHours(0, 0, 0, 0)) return false;
		if (filterEnd && ms > new Date(filterEnd).setHours(23, 59, 59, 999)) return false;
		return true;
	};

	const rows = [];
	let inventoryBatchCount = 0;
	let accountingProductionCost = 0;

	for (const month of collectInventoryMonthsForReports()) {
		let payload = null;
		try {
			payload = JSON.parse(localStorage.getItem(`ww_inventory_${month}`) || 'null');
		} catch (_e) {
			payload = null;
		}
		const finished = Array.isArray(payload && payload.finishedProducts) ? payload.finishedProducts : [];
		for (const product of finished) {
			if (!product || typeof product !== 'object') continue;
			const qty = Number(product.qty || product.quantity || 0) || 0;
			if (qty <= 0) continue;
			const dateText = String(product.date || '').trim() || `${month}-01`;
			const dateMs = parseDateToStartOfDayMs(dateText);
			if (!inRangeMs(dateMs)) continue;
			rows.push({ dateMs, qty, cost: 0, source: 'inventory' });
			inventoryBatchCount += 1;
		}
	}

	const ledger = Array.isArray(accountingData && accountingData.ledger) ? accountingData.ledger : [];
	for (const entry of ledger) {
		if (!entry || typeof entry !== 'object') continue;
		const dateMs = parseDateToStartOfDayMs(entry.date);
		if (!inRangeMs(dateMs)) continue;

		const description = String(entry.description || entry.desc || entry.narration || '').trim();
		const account = String(entry.account || '').trim().toLowerCase();
		const qty = parseAccountingProductionQty(description);
		const looksProduction = /production|packaging|tnt|roll|bag/.test(`${account} ${description.toLowerCase()}`);
		const expenseValue = Math.max(0, (Number(entry.debit) || 0) - (Number(entry.credit) || 0));

		if (qty > 0) {
			rows.push({ dateMs, qty, cost: 0, source: 'accounting' });
		}
		if (looksProduction && expenseValue > 0) {
			accountingProductionCost += expenseValue;
		}
	}

	const byDate = new Map();
	for (const row of rows) {
		if (!row || !Number.isFinite(row.dateMs)) continue;
		const prev = byDate.get(row.dateMs) || { dateMs: row.dateMs, qty: 0, cost: 0 };
		prev.qty += Number(row.qty || 0) || 0;
		prev.cost += Number(row.cost || 0) || 0;
		byDate.set(row.dateMs, prev);
	}

	const mergedRows = Array.from(byDate.values())
		.filter((row) => row.qty > 0 || row.cost > 0)
		.sort((a, b) => a.dateMs - b.dateMs);

	return {
		rows: mergedRows,
		inventoryBatchCount,
		accountingProductionCost,
	};
}

function computeWeeklyOpsAveragesFromRows(invoices, productionRows, options = {}) {
	const syntheticBatches = (Array.isArray(productionRows) ? productionRows : []).map((row) => ({
		date: new Date(row.dateMs).toISOString().slice(0, 10),
		qty: Number(row.qty || 0) || 0,
		cost: Number(row.cost || 0) || 0,
		status: 'completed',
	}));
	return computeWeeklyOpsAverages(invoices, syntheticBatches, options);
}

function computeWeeklyOpsAverages(invoices, batches, options = {}) {
	const recentWeeks = Math.max(1, Math.floor(Number(options.recentWeeks || 8)));
	const paidOnly = options.paidOnly !== false;
	const DAY_MS = 24 * 60 * 60 * 1000;

	const toStartOfDayMs = (rawDate) => {
		const parsed = parseDateLikeToMs(rawDate);
		if (!Number.isFinite(parsed)) return Number.NaN;
		const dt = new Date(parsed);
		dt.setHours(0, 0, 0, 0);
	...
	const ensureWeekBucket = (map, weekMs) => {
		if (!map.has(weekMs)) {
			map.set(weekMs, { produced: 0, sold: 0, revenue: 0, cost: 0 });
		}
		return map.get(weekMs);
	};
	...
	for (const batch of (Array.isArray(batches) ? batches : [])) {
		...
		const dayMs = toStartOfDayMs(batch.date || batch.addedDate);
		if (!Number.isFinite(dayMs)) continue;
		const status = String(batch.status || '').trim().toLowerCase();
		if (status && status !== 'completed') continue;
		const bucket = ensureWeekBucket(weekMap, weekStartMs(dayMs));
		bucket.produced += Number(batch.qty || 0) || 0;
		bucket.cost += Number(batch.cost || 0) || 0;
	}

	for (const invoice of (Array.isArray(invoices) ? invoices : [])) {
		if (!invoice || typeof invoice !== 'object') continue;
		const dayMs = toStartOfDayMs(invoice.date);
		if (!Number.isFinite(dayMs)) continue;
		const status = String(invoice.status || '').trim().toLowerCase();
		if (paidOnly && status !== 'paid') continue;
		const bucket = ensureWeekBucket(weekMap, weekStartMs(dayMs));
		bucket.sold += getInvoiceOutputBags(invoice);
		bucket.revenue += Number(invoice.amount || 0) || 0;
	}

	const activeWeekKeys = [...weekMap.keys()]
		.sort((a, b) => a - b)
		.filter((key) => {
			const row = weekMap.get(key);
			return row && (row.produced > 0 || row.sold > 0 || row.revenue > 0 || row.cost > 0);
		});

	const selectedWeekKeys = activeWeekKeys.slice(Math.max(0, activeWeekKeys.length - recentWeeks));
	if (!selectedWeekKeys.length) {
		return {
			weeks: 0,
			avgProduced: 0,
			avgSold: 0,
	...
	const totals = selectedWeekKeys.reduce((acc, key) => {
		const row = weekMap.get(key) || { produced: 0, sold: 0, revenue: 0, cost: 0 };
		acc.produced += row.produced;
		acc.sold += row.sold;
		acc.revenue += row.revenue;
		acc.cost += row.cost;
		return acc;
	}, { produced: 0, sold: 0, revenue: 0, cost: 0 });

	const weeks = selectedWeekKeys.length;
	const avgProduced = totals.produced / weeks;
	const avgSold = totals.sold / weeks;
	const avgRevenue = totals.revenue / weeks;
	const avgCost = totals.cost / weeks;
	const avgProfit = avgRevenue - avgCost;
	const sellThroughPct = avgProduced > 0 ? (avgSold / avgProduced) * 100 : 0;
	const marginPct = avgRevenue > 0 ? (avgProfit / avgRevenue) * 100 : 0;

	const currentWeekMs = selectedWeekKeys[selectedWeekKeys.length - 1];
	const currentRow = weekMap.get(currentWeekMs) || { produced: 0, sold: 0, revenue: 0, cost: 0 };
	const currentStart = new Date(currentWeekMs);
	const currentEnd = new Date(currentWeekMs + (6 * DAY_MS));
	const labelFormat = { day: '2-digit', month: 'short' };
	const currentWeekLabel = `${currentStart.toLocaleDateString('en-GB', labelFormat)} - ${currentEnd.toLocaleDateString('en-GB', labelFormat)}`;
	...
	return {
		...
		sellThroughPct,
		marginPct,
		currentProduced: currentRow.produced,
		currentSold: currentRow.sold,
		currentProfit: currentRow.revenue - currentRow.cost,
		currentWeekLabel,
	};
}
...
async function initDashboardPage() {
	...
	const inferDashboardFinishedMonth = (row) => {
	...
	const renderDashboardLiveSummary = () => {
		...
		const finishedToday = finishedRows.reduce((sum, item) => {
			...
			let dateKey = '';
			if (/^\d{4}-\d{2}-\d{2}$/.test(rawDate)) dateKey = rawDate;
			else {
				const parsed = new Date(rawDate);
				if (!isNaN(parsed)) dateKey = parsed.toISOString().slice(0, 10);
			}
			if (!dateKey) return sum;
			if (dateKey !== todayStr) return sum;
			if (!isDateInSelectedDashboardYear(dateKey)) return sum;
			return sum + (Number(item.qty) || 0);
		}, 0);
		const logUnitsProduced = Object.values(productionLog).reduce((sum, value) => sum + (Number(value) || 0), 0);
		const totalUnitsProduced = finishedProductsTotal > 0 ? finishedProductsTotal : logUnitsProduced;
		const unitsProducedToday = finishedToday > 0 ? finishedToday : Number(productionLog[todayStr] || 0);
...
function normalizeMonthInvoices(month, invoices) {
	...
	for (const inv of Array.isArray(invoices) ? invoices : []) {
		...
		const txn = getInvoiceTxnKey(inv);
		if (txn) {
			if (seenTxnKeys.has(txn)) continue;
			seenTxnKeys.add(txn);
		}
		keep.push(inv);
	}
...
function mergeSalesRecordsIntoMonth(month, invoices, orders) {
	...
	const existingTxn = new Set(existingInvs.map((inv) => getInvoiceTxnKey(inv)).filter(Boolean));
	const existingOrderTxn = new Set(existingOrds.map((ord) => getOrderTxnKey(ord)).filter(Boolean));
	...
	for (const inv of invoiceList) {
		const id = String(inv.id);
		const txn = getInvoiceTxnKey(inv);
		if (deletedInv.has(id) || existingIds.has(id) || (txn && existingTxn.has(txn))) continue;
		nextInvoices.push(inv);
		existingIds.add(id);
		if (txn) existingTxn.add(txn);
		changed = true;
	}
	for (const ord of orderList) {
		const id = String(ord.id);
		const txn = getOrderTxnKey(ord);
		if (deletedOrd.has(id) || existingOrderIds.has(id) || (txn && existingOrderTxn.has(txn))) continue;
		nextOrders.push(ord);
		existingOrderIds.add(id);
		if (txn) existingOrderTxn.add(txn);
		changed = true;
	}
...
function mergeInvoicesIntoMonthStorage(month, invoices) {
	...
	for (const inv of invoices) {
		...
		const txn = getInvoiceTxnKey(inv);
		if (existingIds.has(id) || (txn && existingTxn.has(txn))) continue;
		additions.push(inv);
		existingIds.add(id);
		if (txn) existingTxn.add(txn);
	}
		...
		try {
			...
			if (stored && typeof stored === 'object') {
				if (Array.isArray(stored.invoices)) {
					const monthInvoices = dedupeInvoiceRows(stored.invoices);
					allInvoices.push(...monthInvoices);
				}
				const monthInvoicesForLink = dedupeInvoiceRows(stored.invoices);
				const monthInvoiceIds = new Set(monthInvoicesForLink.map((inv) => String(inv?.id || '')).filter(Boolean));
				if (Array.isArray(stored.salesOrders)) {
					for (const ord of dedupeOrderRows(stored.salesOrders)) {
						if (!ord || !ord.id) continue;
...
function dedupeSalesMonthPayload(payload) {
	...
	const dedupedInvoices = dedupeInvoiceRows(invoices);

	const activeInvoiceIds = new Set(dedupedInvoices.map((inv) => String(inv.id || '')).filter(Boolean));
	const dedupedOrders = dedupeOrderRows(salesOrders).filter((ord) => {
		if (!ord || !ord.id) return false;
		if (ord.sourceInvoiceId && !activeInvoiceIds.has(String(ord.sourceInvoiceId))) return false;
		return true;
	});
...
function loadMonthData(month) {
	...
	let dirty = false;
	let needsConsistencyRepair = false;
	...
	if (needsConsistencyRepair) {
		const repairStats = repairSalesMonthConsistency(month);
		if (repairStats.afterInvoices !== repairStats.beforeInvoices || repairStats.afterOrders !== repairStats.beforeOrders) {
			dirty = true;
		}
	}
...
async function initSalesInvoicesPage() {
  try {
	...
	const ensureSalesSyncWarningBanner = () => {
		...
		if (!host) return null;
		let banner = document.getElementById('sales-sync-warning-banner');
		if (banner) return banner;
		banner = document.createElement('div');
		banner.id = 'sales-sync-warning-banner';
		banner.style.cssText = 'display:none;margin:0 0 12px 0;padding:10px 12px;border-radius:10px;border:1px solid #fecaca;background:#fff1f2;color:#991b1b;font-size:0.9rem;';
		const firstPanel = host.querySelector('.panel');
		if (firstPanel && firstPanel.parentNode === host) host.insertBefore(banner, firstPanel);
		else host.insertBefore(banner, host.firstChild);
		return banner;
	};

	const renderSalesSyncWarningBanner = () => {
		const banner = ensureSalesSyncWarningBanner();
		if (!banner) return;
		const message = getSalesSyncWarning();
		if (!message) {
		...
		banner.innerHTML = `<strong>Sync blocked:</strong> ${escapeHtml(message)} Use Add Invoice only for new invoices.`;
	};

	renderSalesSyncWarningBanner();
	document.addEventListener('ww-sales-sync-warning', renderSalesSyncWarningBanner);
			...
			if (currentEntity === 'invoice') {
				...
				if (editingSiIdx >= 0 && salesModuleData.invoices[editingSiIdx]) {
					...
					appendPrivilegedActionAudit({
						...
						module: 'invoices',
						entity: 'invoice',
						recordId: String(savedInvoice?.id || ''),
						summary: summarizeAuditRecord(savedInvoice),
						details: 'Edited invoice details',
						month: currentSalesMonth,
					});
				} else {
					...
					if (existingByTxn) {
						...
						appendSalesAddAuditMarker({
							type: 'invoice_add',
							month: currentSalesMonth,
							txnId: submitTxnId,
							invoiceId: invData.id,
							orderId: `SO${String(invData.id).slice(3)}`,
							customer,
						});
					}
				}
			...
			if (currentEntity === 'order') {
				...
				if (editingSiIdx >= 0 && salesModuleData.salesOrders[editingSiIdx]) {
					...
					appendPrivilegedActionAudit({
						...
						module: 'sales',
						entity: 'order',
						recordId: String(linkedOrder?.id || ''),
						summary: summarizeAuditRecord(linkedOrder),
						details: 'Edited sales order details',
						month: currentSalesMonth,
					});
					...
					alert('To prevent duplicate invoices, new records must be created through "Add Invoice" only.');
					return;
				}
			}
...
const DEFAULT_LEDGER_ACCOUNT_OPTIONS = [
	'Utilities',
	'Salaries',
	'Production',
	'Marketing',
...
function getLedgerAccountSuggestions() {
	return [...DEFAULT_LEDGER_ACCOUNT_OPTIONS];
}
	...
	const ACC_MODAL_CONFIGS = {
		ledger: {
			...
			fields: [
				...
				{ id: 'account', label: 'Account', type: 'text', required: true, placeholder: 'Choose or type account', datalistKey: 'ledgerAccounts' },
	...
	const openModal = (entity, editIdx, resumeDraft = false) => {
		...
		if (modalFieldsEl) {
			modalFieldsEl.innerHTML = `<p class="inv-modal-summary-error" id="acc-modal-error-summary" aria-live="polite"></p>` + config.fields.map((f) => {
				...
				if (f.datalistKey === 'ledgerAccounts') {
					const options = getLedgerAccountSuggestions();
					const listId = `acc-field-${f.id}-list`;
					return `<div class="inv-modal-field"><label for="acc-field-${f.id}">${f.label}${f.required ? ' <span class="req">*</span>' : ''}</label><input id="acc-field-${f.id}" type="text" name="${f.id}" list="${listId}" autocomplete="off" spellcheck="false" ${f.required ? 'required' : ''} ${f.placeholder ? `placeholder="${f.placeholder}"` : ''}><datalist id="${listId}">${options.map((o) => `<option value="${escapeHtml(o)}"></option>`).join('')}</datalist><p class="inv-modal-error" id="acc-err-${f.id}" aria-live="polite"></p></div>`;
				}
...
function initProductionPage() {
	...
	const toIsoDateOnly = (rawDate) => {
		const text = String(rawDate || '').trim();
		if (!text) return '';
		if (/^\d{4}-\d{2}-\d{2}$/.test(text)) return text;
		const ms = parseDateLikeToMs(text);
	...
	function collectInventoryFinishedProductsHistory() {
		const rows = [];
		try {
			const merged = JSON.parse(localStorage.getItem('ww_finished_products') || '[]');
			if (Array.isArray(merged)) rows.push(...merged);
		} catch (_e) { /* ignore */ }

		for (let i = 0; i < localStorage.length; i += 1) {
			const key = localStorage.key(i);
			if (!/^ww_inventory_\d{4}-\d{2}$/.test(String(key || ''))) continue;
			try {
				const payload = JSON.parse(localStorage.getItem(key) || 'null');
				const products = Array.isArray(payload && payload.finishedProducts) ? payload.finishedProducts : [];
				rows.push(...products);
			} catch (_e) { /* ignore invalid month payload */ }
		}

		return rows;
	}

	function buildDerivedHistoricalBatches() {
		const finishedRows = collectInventoryFinishedProductsHistory();
		const dedup = new Map();
		for (const row of finishedRows) {
			if (!row || typeof row !== 'object') continue;
			const product = String(row.product || row.name || '').trim();
			const qty = Number(row.qty || row.quantity || 0) || 0;
			const date = toIsoDateOnly(row.date || row.addedDate || row.createdAt || row.updatedAt);
			if (!product || qty <= 0 || !date) continue;
			const unitCost = Number(row.unitCost || row.cost || 0) || 0;
			const key = `${date}|${product.toLowerCase()}`;
			const existing = dedup.get(key) || {
				id: `HIST-${date}-${product.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'batch'}`,
				product,
				qty: 0,
				date,
				shift: 'Historical',
				...
				status: 'completed',
				__derived: true,
				__sourceLabel: 'Inventory & Stock (Finished Products)',
			};
			existing.qty += qty;
			existing.cost += unitCost > 0 ? unitCost * qty : 0;
			dedup.set(key, existing);
		}
		return [...dedup.values()];
	}

	function getBatchesForProductionView() {
		const manualRows = (Array.isArray(prodBatches) ? prodBatches : []).map((row, idx) => ({
			...row,
			__derived: false,
			__manualIndex: idx,
		}));
		const manualSignatures = new Set(manualRows.map((row) => `${toIsoDateOnly(row.date)}|${String(row.product || '').trim().toLowerCase()}`));
		const inventoryDerivedRows = buildDerivedHistoricalBatches();
		const { accountingData } = loadProductionSourceData();
		const accountingDerivedRows = buildAccountingProductionRows(accountingData).map((row) => ({
			id: String(row.id || `ACCT-${row.date}-${String(row.product || '').toLowerCase().replace(/[^a-z0-9]+/g, '-')}`),
			product: String(row.product || 'Production Input').trim(),
			qty: Number(row.qty || 0) || 0,
			date: toIsoDateOnly(row.date) || getTodayDateStr(),
			shift: 'Accounting',
			time: '00:00',
			cost: Number(row.cost || 0) || 0,
			sales: 0,
			status: String(row.status || 'captured').toLowerCase(),
			quality: row.quality || 'Needs Review',
			__derived: true,
			__sourceType: 'accounting',
			__sourceLabel: 'Accounting',
		}));
		const derivedRows = [...inventoryDerivedRows, ...accountingDerivedRows]
			.filter((row) => !manualSignatures.has(`${toIsoDateOnly(row.date)}|${String(row.product || '').trim().toLowerCase()}`));
		const merged = [...manualRows, ...derivedRows];
		return sortRowsChronologically(merged, ['date', 'createdAt', 'updatedAt']);
	}

	function getHistoricalBatchWindowLabel(rows) {
		const dated = (Array.isArray(rows) ? rows : [])
			.map((row) => toIsoDateOnly(row && row.date))
			.filter((text) => /^\d{4}-\d{2}-\d{2}$/.test(text))
			.sort();
		if (!dated.length) return 'No historical batch dates yet';
		return `${formatDateDisplay(dated[0])} to ${formatDateDisplay(dated[dated.length - 1])}`;
	}

	function promoteDerivedBatchToManual(derivedId) {
		const id = String(derivedId || '').trim();
		if (!id) return;
		const derived = getBatchesForProductionView().find((row) => row && row.__derived && String(row.id || '').trim() === id);
		if (!derived) {
			alert('Could not find this imported row.');
			return;
		}
		const signature = `${toIsoDateOnly(derived.date)}|${String(derived.product || '').trim().toLowerCase()}`;
		const exists = (Array.isArray(prodBatches) ? prodBatches : []).some((row) => `${toIsoDateOnly(row && row.date)}|${String(row && row.product || '').trim().toLowerCase()}` === signature);
		if (exists) {
			showDeleteToast('This historical row already exists as a production batch.');
			return;
		}
		prodBatches.push({
			id: nextBatchId(),
			product: String(derived.product || '').trim() || 'Historical Product',
			qty: Number(derived.qty || 0) || 0,
			date: toIsoDateOnly(derived.date) || getTodayDateStr(),
			shift: 'Historical',
			time: '00:00',
			cost: Number(derived.cost || 0) || 0,
			sales: Number(derived.sales || 0) || 0,
			status: 'completed',
		});
		saveBatches();
		showDeleteToast(`Imported ${derived.product} into editable production batches.`);
	}
	...
	function renderProductionPage() {
		const batchRowsForView = getBatchesForProductionView();
		/* Stats — auto-computed from batches + BOM */
		const todayBatches = batchRowsForView.filter((b) => b.date === todayStr);
		const totalBatches = batchRowsForView.length;
		const totalUnits = batchRowsForView.reduce((s, b) => s + (Number(b.qty) || 0), 0);
		const totalCost = batchRowsForView.reduce((s, b) => s + (Number(b.cost) || 0), 0);
		const totalSales = batchRowsForView.reduce((s, b) => s + (Number(b.sales) || 0), 0);
		const productCount = Object.keys(billOfMaterials).length;
		const historicalWindow = getHistoricalBatchWindowLabel(batchRowsForView);
		const stats = document.getElementById('prod-stats-row');
		if (stats) {
			stats.innerHTML = `
				<div class="stat-card"><div class="s-icon"><i class="fa-solid fa-gears"></i></div><p class="s-label">Total Batches</p><p class="s-value">${totalBatches}</p><p class="s-meta">${todayBatches.length} today · ${productCount} product${productCount !== 1 ? 's' : ''}</p><p class="s-meta">History: ${escapeHtml(historicalWindow)}</p></div>
		...
		if (batchBody) {
			...
			batchBody.innerHTML = batchDraftRow + (batchRowsForView.length === 0
				? '<tr><td colspan="9" style="text-align:center;color:#94a3b8;">No batches yet. Click "Add Batch" to start.</td></tr>'
				: batchRowsForView.map((b) => {
					const pillClass = b.status === 'completed' ? 'status-green' : b.status === 'in-progress' ? 'status-blue' : 'status-orange';
					const actions = b.__derived
						? (String(b.__sourceType || '') === 'accounting'
							? `<div class="row-actions"><a class="btn-secondary" href="accounting.html">View Entry</a></div>`
							: `<div class="row-actions"><button class="btn-secondary prod-promote-derived" data-prod-promote-derived="${escapeHtml(String(b.id || ''))}">Use As Batch</button></div>`)
						: `<div class="row-actions"><button class="btn-edit prod-edit-batch" data-idx="${Number(b.__manualIndex)}"><i class="fa-solid fa-pen-to-square"></i></button><button class="btn-delete prod-delete-batch" data-idx="${Number(b.__manualIndex)}"><i class="fa-solid fa-trash"></i></button></div>`;
					const sourceBadge = b.__derived
						? (String(b.__sourceType || '') === 'accounting'
							? ' <span class="status-pill status-yellow" style="margin-left:6px;">From Accounting</span>'
							: ' <span class="status-pill status-blue" style="margin-left:6px;">From Inventory</span>')
						: '';
					return `<tr><td>${escapeHtml(b.id || '')}</td><td>${escapeHtml(b.product || '')}${sourceBadge}</td><td>${formatNumber(Number(b.qty || 0))}</td><td>${escapeHtml(b.date || '')}</td><td>${escapeHtml(b.shift || '')}</td><td>${escapeHtml(b.time || '')}</td><td>${formatCurrency(Number(b.cost || 0))}</td><td><span class="status-pill ${pillClass}">${escapeHtml(String(b.status || 'completed'))}</span></td><td>${actions}</td></tr>`;
				}).join(''));
		}
	...
	document.addEventListener('click', (event) => {
		const promoteDerivedBtn = event.target.closest('[data-prod-promote-derived]');
		if (promoteDerivedBtn) {
			event.preventDefault();
			event.stopPropagation();
			promoteDerivedBatchToManual(String(promoteDerivedBtn.getAttribute('data-prod-promote-derived') || ''));
			renderProductionPage();
			renderProfitabilityChart();
			return;
		}
		const importSuggestionBtn = event.target.closest('[data-prod-import-suggestion]');
		if (importSuggestionBtn) {
			event.preventDefault();
			event.stopPropagation();
			addExternalSuggestionToBom(Number(importSuggestionBtn.getAttribute('data-prod-import-suggestion')));
			return;
		}
	...
	const externalMaterialsList = document.getElementById('prod-external-materials-list');
	let selectedProduct = Object.keys(billOfMaterials)[0];
	let externalSuggestions = [];

	function loadProductionSourceData() {
		let rawMaterials = [];
		let purchaseOrders = [];
		let suppliers = [];
		let accountingData = { ledger: [] };
		try {
			rawMaterials = JSON.parse(localStorage.getItem('ww_raw_materials') || '[]');
			if (!Array.isArray(rawMaterials)) rawMaterials = [];
		} catch (_e) { rawMaterials = []; }
		try {
			const purchasePayload = JSON.parse(localStorage.getItem('ww_purchase_data_v2') || '{}');
			purchaseOrders = Array.isArray(purchasePayload && purchasePayload.purchaseOrders) ? purchasePayload.purchaseOrders : [];
			suppliers = Array.isArray(purchasePayload && purchasePayload.suppliers) ? purchasePayload.suppliers : [];
		} catch (_e) {
			purchaseOrders = [];
			suppliers = [];
		}
		try {
			const stored = JSON.parse(localStorage.getItem('ww_accounting_data_v2') || '{}');
			if (stored && typeof stored === 'object') {
				accountingData = {
					ledger: Array.isArray(stored.ledger) ? stored.ledger : [],
					cashbook: Array.isArray(stored.cashbook) ? stored.cashbook : [],
				};
			}
		} catch (_e) {
			accountingData = { ledger: [], cashbook: [] };
		}
		return { rawMaterials, purchaseOrders, suppliers, accountingData };
	}

	function parseMaterialTextToSuggestions(text, source, options = {}) {
		const preserveLabel = !!(options && options.preserveLabel);
		...
		for (const chunk of chunks) {
			const numberMatches = chunk.match(/\d+(?:\.\d+)?/g) || [];
			const startsWithNumber = /^\s*\d/.test(chunk);
			const endsWithNumber = /\d+(?:\.\d+)?\s*$/.test(chunk);
			let qty = 1;
			let cost = 0;
			if (numberMatches.length >= 2) {
				qty = Number(numberMatches[0]) || 1;
				cost = Number(numberMatches[numberMatches.length - 1]) || 0;
			} else if (numberMatches.length === 1) {
				if (startsWithNumber) qty = Number(numberMatches[0]) || 1;
				if (endsWithNumber && !startsWithNumber) cost = Number(numberMatches[0]) || 0;
			}
			const unitMatch = chunk.match(/\b(rolls?|kg|kgs|g|grams?|pcs?|pieces?|bags?|packs?|cartons?|boxes?|litres?|liters?|ml|l|tnt)\b/i);
			let unit = unitMatch ? String(unitMatch[1] || '').toLowerCase() : 'unit';
			if (unit === 'kgs') unit = 'kg';
			if (unit === 'grams') unit = 'g';
			if (unit === 'pieces') unit = 'pcs';
			if (unit.endsWith('s') && unit !== 'pcs') unit = unit.slice(0, -1);
			let name = preserveLabel ? chunk : chunk.replace(/\d+(?:\.\d+)?/g, ' ');
			if (!preserveLabel && unitMatch) {
				const escaped = String(unitMatch[1] || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
				name = name.replace(new RegExp(`\\b${escaped}\\b`, 'ig'), ' ');
			}
			name = name.replace(/\s+/g, ' ').trim();
			if (!name) name = 'Material';
			rows.push({ name, qty: qty > 0 ? qty : 1, unit, cost: cost > 0 ? cost : 0, source, raw: chunk });
		}
		return rows;
	}

	function isAllowedAccountingProductionLabel(label) {
		const text = String(label || '').replace(/\s+/g, ' ').trim();
		if (!text) return false;
		const lower = text.toLowerCase();
		if (/\b(maintenance|maint|marketing|transport|admin|administration|utility|utilities|salary|salaries|wages)\b/.test(lower)) return false;
	...
	function buildAccountingProductionRows(accountingData) {
		const rows = [];
		const ledger = Array.isArray(accountingData && accountingData.ledger) ? accountingData.ledger : [];
		const appendEntryRows = (entry) => {
			if (!entry || typeof entry !== 'object') return;
			const account = String(entry.account || '').trim().toLowerCase();
			if (account !== 'production') return;
			const description = String(entry.description || entry.desc || entry.narration || '').trim();
			const parsed = parseMaterialTextToSuggestions(description, 'Accounting Production Ledger', { preserveLabel: true });
			const usableParsed = parsed.filter((row) => isAllowedAccountingProductionLabel(row && (row.name || row.raw)));
			const netExpense = Math.max(0, (Number(entry.debit) || 0) - (Number(entry.credit) || 0));
			const date = toIsoDateOnly(entry.date) || getTodayDateStr();

			if (!usableParsed.length) {
				return;
			}

			const distributedCost = netExpense > 0 ? +(netExpense / usableParsed.length).toFixed(2) : 0;
			for (const row of usableParsed) {
				rows.push({
					id: String(entry.id || entry._id || `${date}-ledger`) + '-' + String(row.name || '').toLowerCase().replace(/[^a-z0-9]+/g, '-'),
					date,
					product: String(row.name || 'Production Input'),
					qty: Number(row.qty || 0) || 0,
					cost: Number(row.cost || 0) > 0 ? Number(row.cost || 0) : distributedCost,
					status: 'captured',
					quality: Number(row.qty || 0) > 0 ? 'Verified' : 'Needs Review',
					__sourceType: 'accounting',
				});
			}
		};
		for (const entry of ledger) {
			appendEntryRows(entry);
		}
		return sortRowsChronologically(rows, ['date']);
	}

	function buildExternalMaterialSuggestions() {
		const { rawMaterials, purchaseOrders, suppliers, accountingData } = loadProductionSourceData();
		const rows = [];

		for (const mat of rawMaterials) {
			if (!mat || typeof mat !== 'object') continue;
			const name = String(mat.material || '').trim();
			if (!name) continue;
			rows.push({
				name,
				qty: Number(mat.quantity || 0) || 1,
				unit: 'unit',
				cost: 0,
				source: 'Inventory: Raw Materials',
				raw: `${formatNumber(Number(mat.quantity || 0))} ${name}`,
			});
		}
		...
		for (const po of purchaseOrders) {
			...
			for (const item of items) {
				...
				rows.push({
					...
					raw: `${formatNumber(Number(item.qty || 0))} ${name}`,
				});
			}
		}
		...
		for (const supplier of suppliers) {
			...
			const materialsText = String(supplier.materials || '').trim();
			if (!materialsText) continue;
			rows.push(...parseMaterialTextToSuggestions(materialsText, 'Supplier Materials'));
		}

		for (const row of buildAccountingProductionRows(accountingData)) {
			rows.push({
				name: row.product,
		...
		const dedup = new Map();
		for (const row of rows) {
			const key = `${String(row.name || '').toLowerCase()}|${String(row.unit || 'unit').toLowerCase()}`;
			const existing = dedup.get(key);
			if (!existing) {
				dedup.set(key, { ...row });
				continue;
			}
			existing.qty += Number(row.qty || 0) || 0;
			if (!existing.cost && row.cost) existing.cost = row.cost;
	...
	function getAccountingBomComponents() {
		const { accountingData } = loadProductionSourceData();
		const dedup = new Map();
		for (const row of buildAccountingProductionRows(accountingData)) {
			...
			const existing = dedup.get(key) || {
				...
				unit,
				cost: 0,
				__fromAccounting: true,
			};
			existing.qty += Number(row.qty || 0) || 0;
			existing.cost += Number(row.cost || 0) || 0;
			dedup.set(key, existing);
		}
		return Array.from(dedup.values()).sort((a, b) => String(a.name || '').localeCompare(String(b.name || ''), 'en', { sensitivity: 'base' }));
	}

	function renderExternalMaterialSuggestions() {
		if (!externalMaterialsList) return;
		externalSuggestions = buildExternalMaterialSuggestions();
		if (!externalSuggestions.length) {
			externalMaterialsList.innerHTML = '<p style="margin:0;color:#64748b;">No external production inputs detected yet. Add Inventory/Purchase records first.</p>';
			return;
		}
		externalMaterialsList.innerHTML = externalSuggestions.map((row, idx) => {
			const qty = Number(row.qty || 0) || 0;
			const unit = String(row.unit || 'unit');
			const costText = Number(row.cost || 0) > 0 ? formatCurrency(row.cost) : '—';
			return `
				<div style="display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center;padding:10px 12px;border:1px solid #dbeafe;border-radius:10px;background:#f8fbff;margin-bottom:8px;">
					<div>
						<div style="font-weight:700;color:#0f172a;">${escapeHtml(row.name)}</div>
						<div style="font-size:0.82rem;color:#475569;">Qty: <strong>${escapeHtml(formatNumber(qty))}</strong> ${escapeHtml(unit)} · Cost hint: <strong>${escapeHtml(costText)}</strong></div>
						<div style="font-size:0.77rem;color:#64748b;">Source: ${escapeHtml(row.source)}</div>
						<div style="font-size:0.75rem;color:#94a3b8;">${escapeHtml(row.raw || '')}</div>
					</div>
					<button type="button" class="btn-secondary" data-prod-import-suggestion="${idx}">Add To BOM</button>
				</div>
			`;
		}).join('');
	}

	function addExternalSuggestionToBom(index) {
		if (!Number.isInteger(index) || index < 0 || index >= externalSuggestions.length) return;
		if (!selectedProduct || !billOfMaterials[selectedProduct]) {
			alert('Add or select a product first, then import detected materials.');
			openProductModal();
			return;
		}
		const row = externalSuggestions[index];
		const components = Array.isArray(billOfMaterials[selectedProduct].components) ? billOfMaterials[selectedProduct].components : [];
		const found = components.find((c) => String(c && c.name || '').toLowerCase() === String(row.name || '').toLowerCase());
		if (found) {
			found.qty = (Number(found.qty || 0) || 0) + (Number(row.qty || 0) || 0);
			if (!found.cost && row.cost) found.cost = Number(row.cost || 0) || 0;
			if (!found.unit) found.unit = row.unit || 'unit';
		} else {
			components.push({
				name: row.name,
				qty: Number(row.qty || 0) || 1,
		...
		billOfMaterials[selectedProduct].components = components;
		saveBom();
		renderBom(selectedProduct);
		renderCostAnalysis();
		renderExternalMaterialSuggestions();
		showDeleteToast(`Added "${row.name}" to ${selectedProduct} BOM.`);
	}
	...
	function renderBom(productName) {
		...
		if (!product) {
			...
			if (bomBody) bomBody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:#94a3b8;">No products yet. Click "+ Add Product" on the left to start.</td></tr>';
			if (bomSummary) bomSummary.innerHTML = '';
			return;
		}
		const manualComponents = Array.isArray(product.components) ? product.components : [];
		const accountingComponents = getAccountingBomComponents();
		const useAccountingOnly = accountingComponents.length > 0;
		const visibleComponents = useAccountingOnly
			? accountingComponents
			: manualComponents.map((c, idx) => ({ ...c, __manualIdx: idx, __fromAccounting: false }));

		if (bomTitle) {
			bomTitle.textContent = `${productName} BOM`;
		}
		if (bomBody) {
			bomBody.innerHTML = visibleComponents.length === 0
				? '<tr><td colspan="5" style="text-align:center;color:#94a3b8;">No components yet. Click "Add Component" below.</td></tr>'
				: visibleComponents.map((c) => {
					if (useAccountingOnly || c.__fromAccounting) {
						return `<tr><td>${escapeHtml(c.name)}</td><td>${formatNumber(c.qty)}</td><td>${escapeHtml(c.unit || 'unit')}</td><td>${formatCurrency(c.cost)}</td><td><span class="status-pill status-yellow">From Accounting</span></td></tr>`;
					}
					return `<tr><td>${escapeHtml(c.name)}</td><td>${formatNumber(c.qty)}</td><td>${escapeHtml(c.unit)}</td><td>${formatCurrency(c.cost)}</td><td><div class="row-actions"><button class="btn-edit bom-edit-comp" data-idx="${Number(c.__manualIdx)}"><i class="fa-solid fa-pen-to-square"></i></button><button class="btn-delete bom-delete-comp" data-idx="${Number(c.__manualIdx)}"><i class="fa-solid fa-trash"></i></button></div></td></tr>`;
				}).join('');
		}
		if (bomSummary) {
			const matTotal = visibleComponents.reduce((sum, c) => sum + (Number(c.cost) || 0), 0);
			const accountingMatTotal = useAccountingOnly
				? matTotal
				: accountingComponents.reduce((sum, c) => sum + (Number(c.cost) || 0), 0);
			...
			bomSummary.innerHTML = `
				...
				<div class="detail-row"><span class="detail-key">From Accounting (Production Account)</span><span class="detail-val">${formatCurrency(accountingMatTotal)}</span></div>
	...
	function openCompModal(editIdx) {
		if (!compModal || !compFields) return;
		if (!selectedProduct || !billOfMaterials[selectedProduct]) {
			alert('Add a product first before adding components.');
			openProductModal();
			return;
		}
	...
	function renderCostAnalysis() {
		...
		totalMat += getAccountingBomComponents().reduce((sum, c) => sum + (Number(c.cost) || 0), 0);
	...
	function renderProfitabilityChart() {
		...
		const rows = batchRowsForView.slice(-10).map((b) => ({
			label: b.id,
			revenue: b.sales || 0,
			cost: b.cost || 0,
		}));
	...
	const hydrateProductionSourcesFromServer = async () => {
		try {
			await Promise.all([
				loadFromServerForceFresh(BATCH_KEY),
				loadFromServerForceFresh(BOM_KEY),
				loadFromServerForceFresh('ww_finished_products'),
				loadFromServerForceFresh('ww_raw_materials'),
				loadFromServerForceFresh('ww_purchase_data_v2'),
				loadFromServerForceFresh('ww_accounting_data_v2'),
			]);
			reloadProductionData();
		} catch (_e) { /* keep local cache on failure */ }
	};
	hydrateProductionSourcesFromServer();
	window.addEventListener('focus', hydrateProductionSourcesFromServer);
	window.addEventListener('storage', (e) => {
		if ([BATCH_KEY, BOM_KEY, 'ww_finished_products', 'ww_daily_production', 'ww_raw_materials', 'ww_purchase_data_v2', 'ww_accounting_data_v2'].includes(e.key)
			|| /^ww_inventory_\d{4}-\d{2}$/.test(String(e && e.key || ''))) {
			reloadProductionData();
		}
	});
	try {
		new BroadcastChannel('ww_finished_products_sync').onmessage = reloadProductionData;
		new BroadcastChannel('ww_purchase_sync').onmessage = reloadProductionData;
		new BroadcastChannel('ww_raw_materials_sync').onmessage = reloadProductionData;
		new BroadcastChannel('ww_accounting_sync').onmessage = reloadProductionData;
	} catch (_e) { /* BroadcastChannel not supported */ }
	document.addEventListener('ww-refresh-page', reloadProductionData);
}
...
<head>
  ...
  <style>
    ...
    .export-head p { font-size: 0.85rem; color: #64748b; margin: 0; }
	    ...
	    .panel, .stat-card, .kpi-card-v2, .data-table, .chart-wrap, .rep-performance-card { break-inside: avoid; page-break-inside: avoid; }
	    #rep-tab-financial, #rep-tab-financial .grid-two, #rep-tab-financial .grid-two > div, #rep-tab-financial table, #rep-tab-financial .pie-layout { break-inside: avoid; page-break-inside: avoid; }
	    #rep-tab-financial .grid-two { display: grid !important; grid-template-columns: 1fr 1fr; gap: 14px; }
	    #rep-tab-financial .grid-two > div > h3 { margin-top: 0; }
    .chart-wrap img { max-width: 100%; height: auto; }
	    @media print {
	      @page { size: A4 portrait; margin: 10mm; }
	      body { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
	      .export-wrap { padding: 0; }
	      #rep-tab-financial { break-before: avoid; page-break-before: avoid; }
	    }
  </style>
</head>
<body>
...
function renderReportsData() {
	...
	const productionSource = buildProductionRowsFromInventoryAndAccounting({ filterStart, filterEnd, accountingData: accData });
	const productionRows = Array.isArray(productionSource.rows) && productionSource.rows.length
		? productionSource.rows
		: batches.map((b) => ({
			dateMs: parseDateToStartOfDayMs(b.date || b.addedDate),
			qty: Number(b.qty || 0) || 0,
			cost: Number(b.cost || 0) || 0,
		})).filter((row) => Number.isFinite(row.dateMs) && row.qty > 0);
	...
	const batchDerivedCogs = batches.reduce((s, b) => s + (Number(b.cost) || 0), 0);
	const cogs = Number(productionSource.accountingProductionCost || 0) > 0
		? Number(productionSource.accountingProductionCost || 0)
		: batchDerivedCogs;
	...
	const weeklyOpsFiltered = computeWeeklyOpsAveragesFromRows(invoices, productionRows, { recentWeeks: 8, paidOnly: true });
	const weeklyOpsFallback = (!weeklyOpsFiltered.weeks && filterType !== 'all')
		? computeWeeklyOpsAveragesFromRows(salesData.invoices, productionRows, { recentWeeks: 8, paidOnly: true })
		: weeklyOpsFiltered;
	const weeklyOps = weeklyOpsFallback;
	const weeklyOpsScope = weeklyOpsFiltered.weeks > 0 ? filterLabel : 'All Time';
	const weeklyOpsCardHtml = `
		<div class="stat-card rep-weekly-ops-card">
			<div class="s-icon"><i class="fa-solid fa-calendar-week"></i></div>
			<p class="s-label">Weekly Avg (${weeklyOpsScope})</p>
			<p class="s-value">${formatNumber(Math.round(weeklyOps.avgProduced || 0))}</p>
			<p class="s-meta">Produced bags / week</p>
			<p class="s-meta split-meta">
				<span>Sold: <strong>${formatNumber(Math.round(weeklyOps.avgSold || 0))}</strong> bags</span>
				<span>Profit: <strong>${formatCurrency(weeklyOps.avgProfit || 0)}</strong> / week</span>
				<span>Sell-through: <strong>${(weeklyOps.sellThroughPct || 0).toFixed(1)}%</strong></span>
			</p>
		</div>
	`;
	...
	if (perfHost) {
		...
		const fallbackOutputRows = productionRows.length
			? productionRows.map((row) => ({ date: new Date(row.dateMs).toISOString().slice(0, 10), qty: Number(row.qty || 0) || 0 }))
			: batches.map((row) => ({ date: row && row.date, qty: Number(row && row.qty) || 0 }));
		...
		if (!Number.isFinite(outputStartMs)) {
			...
			const updateRangeButtonLabels = () => {
				...
				const map = currentView === 'monthly'
					? { '7d': '1M', '30d': 'Last 3M', '90d': '6M', '1y': '12M', 'all': 'All' }
					: { '7d': 'Last 7D', '30d': 'Last 30D', '90d': '90D', '1y': '1Y', 'all': 'All' };
				perfRangeWrap.querySelectorAll('.rep-range-btn').forEach((btn) => {
			...
			const rangeToWindow = (rangeKey) => {
				if (currentView === 'monthly') {
					...
					if (rangeKey === '7d') return { min: Math.max(startMonthMs, addMonthsStartMs(endMonthMs, 0)), max: endMonthMs };
				...
				if (rangeKey === '7d') return { min: Math.max(outputStartMs, endMs - (6 * DAY_MS)), max: endMs };
	...
	if (ssEl && dailyShiftSales.length > 0) {
		...
		ssEl.innerHTML = `<p style="color:#64748b;text-align:center;padding:40px 0">No production data for ${filterLabel}.</p>`;
	}
	...
	if (metricCards) {
		const inventoryDerivedBatches = Number(productionSource.inventoryBatchCount || 0);
		const completedBatches = inventoryDerivedBatches > 0 ? inventoryDerivedBatches : batches.filter((b) => b.status === 'completed').length;
		const totalBatches = inventoryDerivedBatches > 0 ? inventoryDerivedBatches : batches.length;
		...
		metricCards.innerHTML = `
			...
			<div class="metric-card"><span class="m-value">${formatNumber(Math.round(weeklyOps.avgProduced || 0))}</span><span class="m-label">Avg Produced / Week</span></div>
			<div class="metric-card"><span class="m-value">${formatCurrency(weeklyOps.avgProfit || 0)}</span><span class="m-label">Avg Profit / Week</span></div>
		`;
	}
}
...
document.addEventListener('DOMContentLoaded', async () => {
	...
	try { bindAuthPanels(); } catch (_e) { /* keep page usable */ }
	try { bindPasswordAssistanceForms(); } catch (_e) { /* keep page usable */ }
